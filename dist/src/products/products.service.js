...
let ProductsService = class ProductsService {
    constructor(prisma, sizeService) {
        this.prisma = prisma;
    ...
    async create(productData, file) {
        const prod = await this.prisma.product.create({
            data: Object.assign(Object.assign({}, productData), { image: file }),
        });
    ...
    async updateById(id, productData, file) {
        const product = await this.prisma.product.findUnique({ where: { id } });
        if (!product)
            throw new common_1.NotFoundException('Product not found');
        if (file) {
            (0, fs_1.unlinkSync)(`./uploads/${product.image}`);
        }
        await this.prisma.product.update({
            where: { id },
            data: Object.assign(Object.assign({}, productData), { image: file ? file : product.image }),
        });
